...
local function parse_args(args)
    ...
    local has_specific_tests = config.run_compare or config.run_rfc or config.run_regression or
                              config.run_bench or config.run_bench_quick or config.run_bench_advanced or
                              config.run_bench_realistic or config.run_bench_compare
...
local function run_unit_tests()
    if not config.run_unit then return true end
...
local function run_functional_tests()
    if not config.run_functional then return true end
    ...
    for _, base_name in ipairs(test_files) do
        ...
        if file_exists(action_file) and file_exists(expected_file) then
            ...
            if config.csv_output then
                log_csv(string.format("functional,%s,%s,%s", test_name, 
                       test_passed and "pass" or "fail", action))
            else
                if test_passed then
...
local function run_compare_tests()
    if not config.run_compare then return true end
    
    log("normal", "\n=== Comparison Tests (vs grepcidr) ===")
    
    -- Check if grepcidr exists
    if not file_exists("grepcidr/grepcidr") then
        log("normal", "Building grepcidr...")
        local output, code = run_command("cd grepcidr && make clean && make")
        if code ~= 0 then
            log("normal", "✗ Failed to build grepcidr")
            log("verbose", output)
            return false
        end
    end
...
local function run_rfc_tests()
    if not config.run_rfc then return true end
...
local function run_regression_tests()
    if not config.run_regression then return true end
...
local function run_benchmarks()
    local any_bench = config.run_bench or config.run_bench_quick or config.run_bench_advanced or 
                     config.run_bench_realistic or config.run_bench_compare
    
    if not any_bench then return true end
