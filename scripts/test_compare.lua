...
local function compare_outputs(test_name, input, pattern, flags)
    ...
    -- Run both commands
    local grepcidr_cmd = string.format("./grepcidr/grepcidr %s %s < %s", flags, pattern, input_file)
...
local function benchmark_comparison(test_name, input, pattern, iterations)
    ...
    for i = 1, iterations do
        run_command(string.format("./grepcidr/grepcidr %s < %s > /dev/null", pattern, input_file))
    end
...
run_command("cd grepcidr && make clean && make")
...
if compare_outputs(
    ...
    "192.168.1.1\n10.0.0.1\n172.16.1.1\n8.8.8.8\n",
    "192.168.0.0/16"
) then tests_passed = tests_passed + 1 end
...
local function generate_mixed_ips(count)
    ...
    for i = 1, count do
        if i % 2 == 0 then
            ...
            table.insert(ips, string.format("2001:db8:%x:%x::%x", 
...
print("\n=== Summary ===")
print(string.format("Functional compatibility: %d/%d tests passed (%.1f%%)", 
    tests_passed, tests_total, tests_passed * 100.0 / tests_total))
